...
class cView{
	...
	MoveCardFromDeck(Card,FieldName){	
		...
		if (Field.className=="Player"){									//Если карта перемещается в руку Player,то показать лицевую сторону карты.
	...
	TakeCard(NamePlayer){
		...
		for(let i=length-1;i>=0;i--){
			...
			if(Player.className=='Computer'){		//Если игрок-компьютер,то перевернуть карту.
				this.HideCard(Ref);
			}
		}
		//Для стола-защиты.
