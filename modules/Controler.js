...
class cController{
	...
	StartGame(){
		...
		let divComputer=this.View.AddElementHTML('.Game','div','Computer',null);
		let divTable=this.View.AddElementHTML('.Game','div','Table',null);
		let divTableAttack=this.View.AddElementHTML('.Table','div','Attack',null);
		let divTableDefend=this.View.AddElementHTML('.Table','div','Defend',null);
		let divCardDeck=this.View.AddElementHTML('.Game','div','CardDeck',null);
		let buttonToMenu=this.View.AddElementHTML('.Game','button','ToMenu','To menu');
		let buttonTake=this.View.AddElementHTML('.Game','button','Take','Take');
		let buttonPass=this.View.AddElementHTML('.Game','button','Pass','Pass');
		let buttonStop=this.View.AddElementHTML('.Game','button','Stop','Stop');
	...
	ProcessingTurn(SelectedCard,Ref){
		...
		if(AttackPlayer==this.ModelPlayer){
			...
			if(Check){										//Если атака произошла,то Computer защищается. 
				if(this.EndOfGame()){						//Проверка на окончание игры.
					return;	
				}
		...
		//Если атакует Computer.
			...
			if(Check){										//Если Player защитился,то Computer может снова атаковать.
				if(this.EndOfGame()){						//Проверка на окончание игры.
					return;
				}
				this.ComputerAttack();
	...
	EndOfGame(){
		...
		if ((this.ModelDeck._deck.length==0)&((this.ModelComputer._deck.length==0)||(this.ModelPlayer._deck.length==0))){
			if(this.ModelComputer._deck.length==0){	//Если рука пуста у Computer,то Player проиграл.
				this.View.ShowAlert('You lost');
			}
			else{									//Если рука пуста у Player,то Player выиграл.
	...
	ComputerAttack(){
		...
		if (SelectedCard!=null){														//Если Computer атаковал
			this.View.MoveCardFromHand(SelectedCard,null,'.Attack',true,'.Computer');	//Перемещение карты из Руки на стол.
		}else{																			//Иначе
	...
	ComputerDefend(AttackCard){
		...
		if(this.ModelPlayer.dopTurn==0){												//Если Player не забрасывает картами Computer.
			SelectedCard=this.ModelComputer.ComputerDefend(AttackCard);					//Computer защищается.
		}
		if(SelectedCard!=null){															//Если Computer защитился.
			this.View.MoveCardFromHand(SelectedCard,Ref,'.Defend',false,'.Computer');	//Перемещение карты из Руки на стол.
			this.OpacityCard();															//Скрытие карт,которыми Player не может атаковать.
		}else{	
			if(this.EndOfGame()){														//Проверка на конец игры.
				return;
			}
	...
	DropCards(){
		...
		//Раздаем карты.
		this.DropCardsPlayer(this.ModelPlayer,'.Player',lengthPlayer0,lengthPlayer1);
		this.DropCardsPlayer(this.ModelComputer,'.Computer',lengthComputer0,lengthComputer1);
	}
	//Раздача карт Игроку.
	...
	PlayerAttack(SelectedCard,Ref){
			...
			if(this.ModelPlayer.CheckContinueTurn()){										//Если не превышен лимит карт на столе.
				Check=this.ModelPlayer.CheckCardAttack(SelectedCard);					
				if(Check){																	//Если Player атаковал,то перемещает карту из Руки на стол. 
					this.View.OpacityCard(Ref,false);										
					this.ModelPlayer.Attack(SelectedCard);									
					this.View.MoveCardFromHand(SelectedCard,Ref,'.Attack',true,'.Player');
	...
	PlayerDefend(SelectedCard,Ref){
			...
			if(Check){																	//Если Player смог защититься,то перемещает карту из Руки на стол. 
				this.ModelPlayer.Defend(SelectedCard);
				this.View.MoveCardFromHand(SelectedCard,Ref,'.Defend',false,'.Player');
			}
			return Check;
	}
	//Смена хода.
	...
	OpacityCard(){
		...
		for(let i=0;i<this.ModelPlayer._deck.length;i++){
			...
			if(this.ModelDeck.Turn==1){																//Если ходит Player.
				if(this.ModelPlayer.CheckContinueTurn()){
					flag=this.ModelPlayer.CheckCardAttack(Card);									//Проверка:можно ли ходить картой.
				}else{
					flag=false;
				}	
			}else{																					//Если ходит Computer.
				RefAttackCard=this.View.ReadCardInHand(this.ModelTable._attack.length-1,'.Attack');	//Вычисление ссылки на последнюю атакующую карту Computer.
			...
			if(flag!=true){																			//Если картой ходить нельзя,то менять внешний вид карты.
