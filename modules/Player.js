...
class cPlayer extends OperationsWithCards {
  ...
  MoveCardFromHand(Card, PlayerAttack) {
    ...
    if (PlayerAttack == true) {
      //Если Игрок атакует,то перемещает в зону атаки.
      this._table.attack = Card;
    } else {
      //Если Игрок защищается,то перемещает в зону защиты.
  ...
  CheckContinueTurn() {
    if (
      (this._enemy._deck.length != 0) &
      (this._table._attack.length < 6) &
      (this._enemy._deck.length + this._table._defend.length >
        this._table._attack.length)
    ) {
      return true;
    }
    return false;
  ...
  CheckCardAttack(SelectedCard) {
    ...
    if (this._table._attack.length != 0) {
      //Если ход не первый.
      for (let i = 0; i < this._table._attack.length; i++) {
        //Поиск карт с таким же рангом.
        if (SelectedCard.range == this._table._attack[i].range) {
          //Если такие карты найдены,то атаковать можно.
          return true;
        }
      }
    } else {
      //Если первый ход,то атаковать можно.
      return true;
    }
    if (this._table._defend.length != 0) {
      for (let i = 0; i < this._table._defend.length; i++) {
        //Поиск карт с таким же рангом.
        if (SelectedCard.range == this._table._defend[i].range) {
          //Если такие карты найдены,то атаковать можно.
          return true;
        }
      }
  ...
  CheckCardDefend(SelectedCard, AttackCard) {
    for (let i = 0; i < this._table._attack.length; i++) {
      //Поиск атакующей карты
      if (this._table._attack[i] == AttackCard) {
        if (SelectedCard.cost > AttackCard.cost) {
          //Если выбранная карта стоит дороже.
          if (SelectedCard.cost > 14) {
            //Если выбранная карта козырь,то защититься можно.
            return true;
          } else if (SelectedCard.suit == AttackCard.suit) {
            //Если карты одной масти,то защититься можно.
            return true;
          }
        }
      }
