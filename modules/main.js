...
let View=new cView();
let Controller=new cController(View,Player,Computer,DeckOnTable,GameTable);
...
document.addEventListener('click',function(event){
	if (event.target.id=='Card'&(event.target.offsetParent!=null)){
		if(event.target.offsetParent.className=='Player'){						//Если выбрана карта,которая находится в руке Player
